...
Keel watches mailbox/outbox/ so you don't have to remember the shipping command.
...
If you forget the cubit send step, drop the file in mailbox/outbox/.
Keel ships it within a second. Frontmatter rules above still apply.

**Only the literal path mailbox/outbox/ is recognized.** Do NOT invent
out/, outgoing/, messages/, send/, queue/, or a workspace-root outbox/ —
they will NOT be picked up.

## What mailbox/outbox/ will NOT ship
...
- type: delegation in mailbox/outbox/ (use cubit delegate)
- Malformed frontmatter (unterminated --- block)

Validation failures rename the draft in-place to mailbox/outbox/<name>.invalid.md
(or mailbox/outbox/<name>.invalid/ for directory drafts), with the reason as the
first line of the file. To recover: fix the issue, drop the .invalid suffix
(rename back to mailbox/outbox/<name>.md), and save — keel auto-retries on save.
...
is reachable again, or move the file back to mailbox/outbox/ for auto-retry.
