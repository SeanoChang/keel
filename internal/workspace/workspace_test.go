...
func TestAppendInbox(t *testing.T) {
	dir := t.TempDir()
	err := AppendInbox(dir, false, "sean", "Focus on the CTA section")
	if err != nil {
		t.Fatal(err)
	}
	content, err := ReadInbox(dir)
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(content, "Focus on the CTA section") {
		t.Error("inbox message not found")
	}
	if !strings.Contains(content, "note from sean") {
		t.Error("expected 'note from sean' in header")
	}
}

func TestAppendInboxPriority(t *testing.T) {
	dir := t.TempDir()
	err := AppendInbox(dir, true, "sean", "Deadline moved to Friday")
	if err != nil {
		t.Fatal(err)
	}
	content, _ := ReadInbox(dir)
	if !strings.Contains(content, "priority from sean") {
		t.Error("expected 'priority from sean' in header")
	}
}

func TestReadInboxMissing(t *testing.T) {
	dir := t.TempDir()
	content, err := ReadInbox(dir)
	if err != nil {
		t.Fatalf("expected no error for missing INBOX.md, got %v", err)
	}
	if content != "" {
...
func TestClearInbox(t *testing.T) {
	dir := t.TempDir()
	os.WriteFile(filepath.Join(dir, "INBOX.md"), []byte("some content"), 0644)

	if err := ClearInbox(dir); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(filepath.Join(dir, "INBOX.md")); !os.IsNotExist(err) {
		t.Error("expected INBOX.md to be removed")
	}

	// Clearing when absent should not error
	if err := ClearInbox(dir); err != nil {
		t.Errorf("clearing absent INBOX.md should not error: %v", err)
	}
}

func TestHasInbox(t *testing.T) {
	dir := t.TempDir()
	if HasInbox(dir) {
		t.Error("expected false for missing INBOX.md")
	}

	os.WriteFile(filepath.Join(dir, "INBOX.md"), []byte(""), 0644)
	if HasInbox(dir) {
		t.Error("expected false for empty INBOX.md")
	}

	os.WriteFile(filepath.Join(dir, "INBOX.md"), []byte("some content"), 0644)
	if !HasInbox(dir) {
		t.Error("expected true for non-empty INBOX.md")
	}
}
